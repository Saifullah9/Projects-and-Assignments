...
/*I added this helper method
*/
v2f pixLoc(float px, float py, float screen_W, float screen_H, float px_of, float py_of) {
	v2f pLoc(0.0, 0.0);
	px = 2 * (px + px_of) / (screen_W)-1;
	py = 2 * (py + py_of) / (screen_H)-1;
	pLoc[0] = px;
	pLoc[1] = -py;
	return pLoc;
}
void Renderer::render() {
    if (realTime) {
        ...
        // TODO(A1): Implement this
////////////////////
		TinyRender::Camera camera = scene.config.camera;
		v3f eye = camera.o;
		v3f at = camera.at;
		v3f up = camera.up;
		float fov = camera.fov;
		float scaling = tan(deg2rad * (fov) / 2.f);
...
////////////////////
		glm::mat4 inverseView = glm::inverse(glm::lookAt(eye, at, up)); // Inverse of world to camera transform -> Camera to World transform
		...
		ThreadPool::SequentialFor(0, scene.config.width, [&](int x) {
			...
			Sampler sampler(260733168 + x);
			for (int y = 0; y < height; ++y) {
				v3f p_avg = v3f(0.f, 0.f, 0.f);
				for (int i = 0; i < spp; ++i) {
					float x_of = 0.5f;
					float y_of = 0.5f;
					if (spp > 1) {
					...
					v2f pLoc = pixLoc(x, y, width, height, x_of, y_of);
					float px = pLoc[0];
					float py = pLoc[1];
					px *= scaling * aspectRatio;
					py *= scaling;
					v4f p_homog = v4f(px, py, -1.0f, 0.0f);
					v4f d = inverseView * p_homog;
					d = normalize(d);
					Ray ray = Ray(eye, d);
					v3f v = integrator->render(ray, sampler);
					p_avg += v;
				}
				p_avg /= spp;
				integrator->rgb->data[x + y * width] = p_avg;
			}
