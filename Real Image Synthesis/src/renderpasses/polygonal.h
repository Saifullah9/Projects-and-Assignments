...
struct PolygonalPass : RenderPass {
    ...
    bool init(const Config& config) override {
		...
		sampler = new Sampler(260733168);
		emitter = scene.emitters[0];
		auto shape = scene.worldData.shapes[emitter.shapeID];
		...
		nbTriangles = shape.mesh.indices.size() / 3;
		for (int i = 0; i < shape.mesh.indices.size(); i++) {
			v3f v_pos = scene.getObjectVertexPosition(emitter.shapeID, i);

			emitterVertexData.push_back(v_pos.x);
			emitterVertexData.push_back(v_pos.y);
			emitterVertexData.push_back(v_pos.z);
		}
    ...
    void addShadows() {
		...
		float spp = float(scene.config.spp);

		glm::mat4 inv_view = glm::inverse(glm::lookAt(camera.camera_position,
			camera.camera_look_at, camera.camera_up)); 
		...
		for (int y = 0; y < scene.config.height; y++) {
			...
			for (int x = 0; x < scene.config.width; x++) {
				...
				v3f dlt = v3f(0.0f);
				float x_offset, y_offset = 0.5f;
				float p_x = 2 * (x + x_offset) / (scene.config.width)-1;
				float p_y = 2 * (y + y_offset) / (scene.config.height) - 1;

				p_x *= scale * aspectRatio;
				p_y *= scale;

				v4f p_homo = v4f(p_x, p_y, -1.0f, 0.0f);

				// Convert direction to world space
				v4f dir = inv_view * p_homo;
				dir = normalize(dir);
				...
				if (scene.bvh->intersect(ray, data)) {
					...
					dlt = polygonalIntegrator->estimateVisDiffRealTime(ray, *sampler, emitter);
				}
		...
		for (int j = 0; j < numDataValues; j++) {
			if (samplesAccumulated - 1 == 0) {
				cvTermData[j] = t[j];
			}

			else {
				cvTermData[j] = ((samplesAccumulated - 1) * cvTermData[j] + t[j]) / samplesAccumulated;
			}
		}
    ...
    void render() override {
        ...
        for (auto& object : objects) {
            ...
            */
			...
			else if (object.shaderIdx == DIFFUSE_SHADER_IDX) {
