...
public class Chaining {
    ...
    public int chain (int key) {
    	...
    	// the formula for function is h(k) = ((A*key) mod 2^w >> w-r
    ...
    public int insertKey(int key){
    	...
    	if(Table.get(hashVal).size() != 0) {
    		
    		int existingKeys = Table.get(hashVal).size();
    		collisions = existingKeys;
    	}
    	
    	Table.get(hashVal).add(key);
